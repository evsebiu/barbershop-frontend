...
const AppointmentForm = () => {
  ...
  useEffect(() => {
    if (barberId) {
      fetch(`http://192.168.1.132:8080/api/barbers/${barberId}`)
        .then(res => res.json())
  ...
  useEffect(() => {
    if (selectedDate) {
      fetch(`http://192.168.1.132:8080/api/appointments/slots?barberId=${barberId}&serviceId=${serviceId}&date=${selectedDate}`)
        .then(res => res.json())
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await fetch('http://192.168.1.132:8080/api/appointments', {
        method: 'POST',
